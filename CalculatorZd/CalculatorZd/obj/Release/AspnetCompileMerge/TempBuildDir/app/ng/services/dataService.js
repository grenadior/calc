(function () {
    ...
    function dataService($http, $q) {
        ...
        var gSearchRequestsBeySession = 0;
        var newDateBegin;
        var secEnd;
        var millEnd;
        function getSearchRequestBySessionId(sessionId, vm, onlyRowsCount, pageId) {
          //  gSearchRequestsBeySession = parseInt(gSearchRequestsBeySession) + 1;
            //30*7=210 minutes
            //if (gSearchRequestsBeySession > 20) {
            ...
            getSearchResultBySessionId(sessionId, pageId).then(function (data) {
                if (data != null ) {
                        if (data.statusSearch == 0) {
                            return;
                        }

                        calculateTimeWorkQuery(data, vm, onlyRowsCount);
                        if (data.statusSearch == 1) {
                            alert('По вашему запросу ничего не найдено');
                        } else if (data.statusSearch == 2) {
                            alert('По вашему запросу найдено: ' + data.totalRecords + ' записей');
                        } else if (data.statusSearch == 3) {
                            alert('Ваш запрос содержит слишкой большой объем данных. Обратитесь в службу поддержки');
                        } else if (data.statusSearch == 4) {
                            alert('Ошибка формирования отчета. Обратитесь в службу поддержки');
                        }
                }
        ...
        function calculateTimeWorkQuery(data, vm, onlyRowsCount) {
